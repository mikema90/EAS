...
$(document)
		.ready(
				...
				function() {
					...
					var myDate = new Date();
					yearSelector.find("[value='" + myDate.getFullYear() + "']")
					...
					if (infoId == null) {
						...
						$("#thesisTypeSelector").find(
								"option:contains('" + thesisInfo.category
										+ "')").attr("selected", "selected");
						insertAuthor2nd(thesisInfo.first_author,
								thesisInfo.first_author_wid);
						if (thesisInfo.other_authors != "") {
							var otherAuthorNames = thesisInfo.other_authors
									.split(",");
							var otherAuthorIds = thesisInfo.other_authors_wid
									.split(",");
							for (var i = 0; i < otherAuthorNames.length; i++) {
								insertAuthor2nd(otherAuthorNames[i],
										otherAuthorIds[i]);
							}
						}
						...
						$(
								".timeSelector.timeOfYear option[value='"
										+ thesisInfo.post_date.split("-")[0]
										+ "']").attr("selected", "selected");
						$(
								".timeSelector.timeOfMonth option[value='"
										+ parseInt(thesisInfo.post_date
												.split("-")[1]) + "']").attr(
								"selected", "selected");
						$(
								"#languageSelector option:contains('"
										+ thesisInfo.language + "')").attr(
								"selected", "selected");
...
function insertAuthorInfo() {
	if ($("#authorNameInputBox").val() == "") {
		...
		insertAuthor2nd($("#authorNameInputBox").val(), $("#authorIdInputBox")
				.val())
		hideAuthorEditor();
...
function onChangeSn2(targetElement) {
	if ($(targetElement).val().length >= 4
			&& $("#periodicalSelector").val() != "CN") {
		$(targetElement).val($(targetElement).val().substr(0, 4));
	} else if ($(targetElement).val().length >= 7
			&& $("#periodicalSelector").val() == "CN") {
...
function checkPeriodicalSn(targetElement) {
	if ($("#periodicalNameInputBox").val() != null
			&& $(".periodicalSn.sn1").val() != null) {
		$.ajax({
			type : 'POST',
			url : '../mapping',
			data : {
				periodicalName : $("#periodicalNameInputBox").val(),
...
function submitForm() {
	if ($("#authorListTable tbody").find("tr").length < 4) {
		...
		alert("论文和期刊名称不能为空");
			...
			&& ($(".periodicalSn.sn1").val().length != 4 || $(
					".periodicalSn.sn2").val().length != 4)) {
		alert("ISSN需要8位数字或字母并且由短横隔开");
	} else if ($("#periodicalSelector").val() == "ISBN"
			&& $(".periodicalSn.sn1").val().length != 13) {
		alert("ISSN需要13位数字");
	} else if ($("#periodicalSelector").val() == "CN"
			&& ($(".periodicalSn.sn1").val().length != 2 || ($(
					".periodicalSn.sn2").val().length != 2
					&& $(".periodicalSn.sn2").val().length != 4 && $(
					".periodicalSn.sn2").val().length != 7))) {
